(function($, undefined) {
    ...
    kendo.data.binders.format = kendo.data.Binder.extend( {
        refresh: function() {
            ...
            if (value) {
                $(this.element).text(kendo.toString(value, "#"));
            }
        }
    });
    ...
    function showHomeView(e) {
        ...
        viewModel.dataSource.filter({ field: "remembered", operator: "eq", value: false});
    }

    function showMenuView() {
        viewModel.dataSource.filter([]);
        viewModel.dataSource.group({field: "difficulty"});
    } 
